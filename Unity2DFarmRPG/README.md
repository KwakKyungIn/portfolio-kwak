...
* **플레이어 루프**: 이동 → 상호작용(Interactable) → 툴/타일/자원 → 결과(작물 성장, 아이템 드랍)
* **월드 시스템**: CropsManager/ResourceNode → TimeAgent 기반 이벤트 처리
* **아이템 루프**: 드랍/습득 → 인벤토리 → 툴바 → 사용/소모/재드랍
* **코어 매니저**: GameManager(전역), SceneManager(씬), TimeAgent(시간), ItemSpawnManager(드랍)
